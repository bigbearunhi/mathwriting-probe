"""Resume the 50k checkpoint; 2048 validation samples, microbatch16, batch256."""
...
config=dict(state['config']);config.update(steps=75000,microbatch=16,effective_batch=256,eval_every=500,validation_ids=ids,valid_samples=2048,source=str(source),patience_steps=None,best_criterion='minimum token_error_rate, higher exact_match breaks ties')
(out/'config.json').write_text(json.dumps(config,indent=2))
def status(stage,step,**kw):
 p=out/'status.partial';p.write_text(json.dumps(dict(stage=stage,step=step,target=75000,updated_unix=time.time(),**kw),indent=2));os.replace(p,out/'status.json')
status('baseline_validation',50000)
v=evaluate(model,data,ids,'cuda',16)
(out/'validation_050000.json').write_text(json.dumps(v,indent=2))
best=(-v['token_error_rate'],v['exact_match'])
save(out/'latest.pt',model,opt,50000,config,rng);shutil.copy2(out/'latest.pt',out/'best.pt')
(out/'best.json').write_text(json.dumps(dict(step=50000,exact_match=best[1],token_error_rate=-best[0])))
print(json.dumps(dict(step=50000,baseline={k:x for k,x in v.items() if k!='examples'})),flush=True)
del state
start=time.monotonic()
for step in range(50001,75001):
...
else:status('completed',75000,validation=summary)
