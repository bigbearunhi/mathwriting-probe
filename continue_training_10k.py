...
import json,random,time,argparse,shutil,os
from pathlib import Path
import torch
from ctc_train import Data,CTCModel,collate,loss_for,evaluate,precision,save

parser=argparse.ArgumentParser(); parser.add_argument('--steps',type=int,default=10000); args=parser.parse_args()
...
baseline=json.loads((out/f"validation_{state['step']:06d}.json").read_text())
best=(baseline['exact_match'],-baseline['token_error_rate'])
if not (out/'best.pt').exists():
 shutil.copy2(out/'diagnostic.pt',out/'best.pt')
 (out/'best.json').write_text(json.dumps(dict(step=state['step'],exact_match=best[0],token_error_rate=-best[1])))
else:
 b=json.loads((out/'best.json').read_text());best=(b['exact_match'],-b['token_error_rate'])
print(json.dumps(dict(resumed_step=state['step'],target=args.steps,learning_rate=opt.param_groups[0]['lr'])),flush=True)
...
for step in range(state['step']+1,args.steps+1):
 ...
 if step%500==0 or step==args.steps:
  ...
  if score>best:
   shutil.copy2(out/'diagnostic.pt',out/'best.partial')
   os.replace(out/'best.partial',out/'best.pt')
   best=score
