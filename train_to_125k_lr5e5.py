"""Resume the best 117k checkpoint at half learning rate; 2048 validation samples, microbatch16, batch256."""
...
out=Path('runs/ctc-125k-lr5e5');out.mkdir(exist_ok=True)
if (out/'config.json').exists():raise RuntimeError('Run already exists; do not overwrite')
source=Path('runs/ctc-125k/best.pt')
state=torch.load(source,map_location='cpu',weights_only=False)
assert state['step']==117000
...
for group in opt.param_groups: group['lr']=.00005
assert all(g['lr']==.00005 for g in opt.param_groups)
torch.set_rng_state(state['torch_rng']);torch.cuda.set_rng_state_all(state['cuda_rng'])
config=dict(state['config']);config.update(lr=.00005,steps=125000,microbatch=16,effective_batch=256,eval_every=500,validation_ids=ids,valid_samples=2048,source=str(source),patience_steps=None,best_criterion='minimum token_error_rate, higher exact_match breaks ties')
...
status('baseline_validation',117000)
v=evaluate(model,data,ids,'cuda',16)
(out/'validation_0117000.json').write_text(json.dumps(v,indent=2))
best=(-v['token_error_rate'],v['exact_match'])
save(out/'latest.pt',model,opt,117000,config,rng);shutil.copy2(out/'latest.pt',out/'best.pt')
(out/'best.json').write_text(json.dumps(dict(step=117000,exact_match=best[1],token_error_rate=-best[0])))
print(json.dumps(dict(step=117000,baseline={k:x for k,x in v.items() if k!='examples'})),flush=True)
del state
start=time.monotonic()
for step in range(117001,125001):
