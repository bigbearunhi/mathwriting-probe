...
p=sorted(Path('runs/ctc-50k').glob('validation_*.json'))[-1];r=json.loads(p.read_text());e=next(e for e in r['examples'] if e['truth']!=e['prediction'])
ns='{http://www.w3.org/2003/InkML}'
with tarfile.open('data/mathwriting-2024.tgz','r|gz') as t:
 for member in t:
  if Path(member.name).parent.name=='valid' and Path(member.name).stem==e['id']:
...
im=Image.new('RGB',(1100,600),'white');d=ImageDraw.Draw(im)
f=ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf',24)
d.text((35,20),p.stem+' | ID: '+e['id'],font=f,fill='black')
pts=[v for s in strokes for v in s];xmin=min(v[0] for v in pts);ymin=min(v[1] for v in pts);dx=max(v[0] for v in pts)-xmin;dy=max(v[1] for v in pts)-ymin;scale=min(960/max(dx,1),320/max(dy,1))
for s in strokes:
...
im.save('runs/error-evidence.png');Path('runs/error-evidence.json').write_text(json.dumps(dict(validation=str(p),**e),indent=2))
